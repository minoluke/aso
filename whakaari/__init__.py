...
class ForecastModel(object):
    ...
    def plot_accuracy(self, ys, save=None):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            MCC = np.where(denominator == 0, np.nan, (TPs*TNs - FPs*FNs) / denominator)
        F1 = 2*TPs/(2*TPs+FPs+FNs)
        accuracy = (TPs+TNs)/(TPs+TNs+FPs+FNs)


           # ゼロ除算を避けるための処理を追加
        false_alert_rate = np.divide(FPs, FPs+TPs, out=np.zeros_like(FPs, dtype=float), where=(FPs+TPs)!=0)
        true_positive_rate = np.divide(TPs, TPs+FNs, out=np.zeros_like(TPs, dtype=float), where=(TPs+FNs)!=0)
        false_positive_rate = np.divide(FPs, FPs+TNs, out=np.zeros_like(FPs, dtype=float), where=(FPs+TNs)!=0)
        ...
        ax1.plot(thresholds, false_alert_rate, 'k-', label='false alert rate')
        ...
        ax1.legend()
        ax2.plot(false_positive_rate, true_positive_rate, 'k-')
        ax2.set_title('ROC')
