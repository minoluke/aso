...
class TremorData(object):
    ...
    def _assess(self):
        ...
        with open(eruptive_periods_path, 'r') as fp:
            self.tes = [datetimeify(ln.rstrip()) for ln in fp.readlines()]
        
        # Check if tremor data file exists
        self.exists = os.path.isfile(self.file)
        if not self.exists:
            # Initialize with a default date range if file doesn't exist
    ...
    def _is_eruption_in(self, days, from_time):
        ...
        """
        for te in self.tes:
            delta_days = (te - from_time).total_seconds() / (3600 * 24)
